...
--                 traders, accounts, deposit count, and total USDC deposited.
--                 Ideal backing query for a metric-card visual.
-- Source        : nft.transfers (trades) 
--                 + dojifunded_arbitrum.dojitradenft_call_minttrade (accounts)
--                 + erc20_arbitrum.evt_Transfer (deposits).
--
-- Method        : Three independent single-row CTEs (trades, accounts, deposits) 
--                 joined with CROSS JOINs. Each side is 1 row, so the cross join 
--                 yields exactly one combined row.
--
-- Addresses
...
accounts AS (
    -- Total unique evaluation accounts created (identified by UUID accountId).
    -- Requires the decoded call table.
    SELECT 
        COUNT(DISTINCT json_extract_scalar(data, '$.accountId')) AS total_eval_accounts
...
deposits AS (
    ...
    WHERE "to" = 0x98D4077A5C448529d20D233d36780e3A99dB541E   -- Reserve A only
...
SELECT
    t.total_trades,
    t.unique_traders,
    a.total_eval_accounts,
    d.total_deposits,
    d.unique_depositors,
    d.total_usdc_deposited
FROM trades t
CROSS JOIN accounts a
CROSS JOIN deposits d   -- yields exactly one combined summary row
